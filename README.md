...
| `npm run write` | **글쓰기 에디터** → http://localhost:3001 |
...
- 수식은 `$인라인$`, 블록은 **`$$` 를 각각 제 줄에** 두어야 합니다:
  ...
  여는 `$$` 뒤에 같은 줄에 내용을 쓰면 remark-math가 그걸 코드펜스의 언어 표시처럼
  **메타 문자열로 보고 버리고**, 닫는 `$$` 도 제 줄에 혼자 있어야만 인정합니다.
  잘못 쓰면 블록이 안 닫혀서 **그 아래 문서를 통째로 삼킵니다.** 에디터로 저장하면
  자동으로 정리되지만, 손으로 쓸 때는 지켜야 합니다.
...
```bash
npm run write     # http://localhost:3001
npm run dev       # 옆에 같이 띄워두면 실제 렌더를 확인할 수 있습니다
```

왼쪽에 마크다운, 오른쪽에 미리보기. 상단에 제목·슬러그·요약·날짜·태그·초안
입력란이 있고, 저장하면 `content/blog/<슬러그>.mdx` 로 떨어집니다. 기존 글은
좌상단 드롭다운에서 골라 불러옵니다.

**이미지는 세 가지 방법 다 됩니다** — 드래그앤드롭, `Ctrl+V` 붙여넣기(스크린샷),
`🖼 이미지` 버튼. 어느 쪽이든 `content/blog/` 에 저장되고 마크다운이 커서 위치에
삽입됩니다. `public/blog-assets/` 에도 즉시 복사돼서 **dev 서버 재시작 없이 바로
보입니다.**

**표**는 `⊞ 표` 버튼에서 칸 수를 골라 잡으면 마크다운 표 뼈대가 들어갑니다.
그 외 툴바: 제목, 굵게/기울임, 인라인 코드, 목록, 인용, 링크, 코드 블록, 수식,
구분선. 단축키는 `Ctrl+B` `Ctrl+I` `Ctrl+K` `Ctrl+S`.

편집 동작:

- **`Ctrl+Z` 정상 동작.** 모든 삽입이 `execCommand("insertText")` 를 거칩니다 —
  `setRangeText` 나 `.value` 대입은 브라우저 되돌리기 스택을 우회해서, 툴바를 한 번
  쓰는 순간 `Ctrl+Z` 가 죽습니다.
- **`Tab` 들여쓰기**, `Shift+Tab` 내어쓰기
- **`Enter` 로 목록·인용 이어쓰기.** 빈 항목에서 한 번 더 누르면 목록을 빠져나갑니다
- 툴바 오른쪽에 **글자수와 예상 읽기시간** (사이트와 같은 방식으로 계산)
- **`◧ 미리보기` 로 오른쪽 창을 접습니다** (`Ctrl+\`). 접힌 상태는 기억되고, 접혀
  있는 동안에는 마크다운·KaTeX 처리를 아예 건너뜁니다

안전장치:

- **저장 안 된 변경은 제목 옆 주황 점**으로 계속 떠 있습니다
- **1초마다 localStorage에 임시저장**됩니다. 탭이 닫히거나 브라우저가 죽어도 다음에
  열 때 복구할지 물어봅니다
- **슬러그를 바꿔 저장하면 옛 파일이 지워집니다** (중복 생성 방지). 다른 글 위에
  덮어쓰려 하면 먼저 물어봅니다
- **삭제** 버튼은 불러온 글이 있을 때만 나타나고, 확인을 거칩니다. 딸린 이미지
  파일은 남으니 필요하면 직접 지우세요

### 왜 Next 앱이 아니라 별도 서버인가

`output: "export"` 는 디스크에 쓰는 라우트 핸들러를 금지합니다. `src/app/` 안에
두면 **빌드가 깨지거나, 안 깨지면 `out/` 에 실려 배포됩니다.** `scripts/` 에 독립
Node 서버로 두면 사이트 빌드와 아예 만나지 않습니다 — 산출물을 grep 해서 확인한
사항입니다.

루프백(`127.0.0.1`)에만 바인딩합니다. 파일을 쓰는 도구라 네트워크에 열리면 안 됩니다.

미리보기는 `marked` + KaTeX로 그려서 **실제 렌더와 완전히 같지는 않습니다**
(Shiki 문법 강조, 정확한 자간 등). 최종 확인은 `localhost:3000` 에서 하세요.
...
- **모든 로고를 높이 28px로 통일합니다.** 비율이 뭐든 높이가 같아야 한 줄로 정렬돼
  보입니다. 폭 상한(210px)은 디자인 목표가 아니라 안전장치입니다 — 7.5:1보다 넓은
  로고만 걸리고, 그 경우엔 28px보다 작게 줄어듭니다. 현재 가장 넓은 DBpia 로고가
  6.67:1로 187px이니 실제로는 안 걸립니다.
- **투명한 로고는 그대로, 투명이 안 되는 로고만 흰 타일에 올립니다.** JPEG는 알파
  채널이 아예 없고 PNG도 알파 없이 저장되는 경우가 많은데, 그런 파일은 흰 사각형을
  달고 다녀서 다크 테마에서 튑니다. 판별은 확장자 추측이 아니라
  [`image-alpha.ts`](src/lib/image-alpha.ts) 가 **파일 헤더에서 알파 채널을 직접
  읽어서** 합니다 (PNG는 IHDR 컬러타입, WebP는 VP8X 플래그).
